...
async def cmd_addchannel(message: Message, command: CommandObject, session: AsyncSession):
    ...
    try:
        channel = await get_channel_manager().add_channel(session, tg_id)
...
async def cmd_channels(message: Message, session: AsyncSession):
    channels = await get_channel_manager().list_channels(session)
    if not channels:
        await message.answer("Каналы не зарегистрированы. Используйте /addchannel [chat_id].")
        return
    lines = [
...
async def cmd_removechannel(message: Message, command: CommandObject, session: AsyncSession):
    ...
    try:
        ...
        return
    ok = await get_channel_manager().remove_channel(session, cid)
...
async def cb_channels(call: CallbackQuery, session: AsyncSession):
    channels = await get_channel_manager().list_channels(session)
    if not channels:
        await call.message.answer("Каналов нет.")
    else:
